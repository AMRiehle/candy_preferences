...
// create nested Object with candies and calculated means
...
// create an array of candy objects where each object is a key-value pair for use in building bar chart

var candies = Object.keys(candy_nest);
var candy_means = Object.values(candy_nest);
var candy_agg = [];
...
// calculate number of entries for each age bracket and put into nested object
...
// calculate number of entries for each gender value and put into nested object
...
// set priority for bars in Countries chart

country_priority = ["United States", "Canada", "Other"];

// calculate number of entries for each country value and put into nested object
...
// create function to aggregate data based on which dataset is being used

function consolidate_data(csv) {

// calculate number of entries in each age bracket and put into nested object
...
// set priority for bars in Countries bar chart
...
// set domains for x-value scales for age, gender, and country based on max number of entries for each demographic
...
// create x-axes for age, gender, and country charts

var x_axis_age = d3.axisBottom(xScale_age);
var x_axis_gender = d3.axisBottom(xScale_gender);
var x_axis_country = d3.axisBottom(xScale_country);

// adjust x-axis for age chart
...
// adjust x-axis for gender chart
...
// adjust x-axis for country chart
...
// create holding arrays for age, gender, and country values that exist in filtered data

var ages = [];
var genders = [];
var countries = [];

// fill in newly-created arrays with values from nested objects
...
// determine width of new chart bars based on nested value for demographic and whether demographic exists for its category
...
// create x- and y-scales for each chart's data
...
// create variables to refer to each chart box
...
// set functionality for "Clear Selected" button for age chart
// changing button color on mouseover/mouseout
// filtering and aggregating data and resetting bar widths and color on click
...
// create functionality for "Clear Selected" button for gender chart
...
// create functionality for "Clear Selected" button for country chart
...
// create functionality for "Clear Selected" button for candy chart
// reset color of bars
// unset chosen_candy variable value
// clear values from right column and reset to grayed-out opacity
...
// set functionality for "Clear All" button
// combination of functionality for "Clear Selected" buttons
...
// create chart axes
    ...
    var y_axis_age = d3.axisLeft(yScale_age);
    var y_axis_gender = d3.axisLeft(yScale_gender);
    var y_axis_country = d3.axisLeft(yScale_country);
    var y_axis_candy = d3.axisLeft(yScale_candy);

    var x_axis_age = d3.axisBottom(xScale_age);
    var x_axis_gender = d3.axisBottom(xScale_gender);
    var x_axis_country = d3.axisBottom(xScale_country);
    var x_axis_candy = d3.axisBottom(xScale_candy);

// apply chart axes to charts
...
// set default values for click flags

ageClicked = false;
genderClicked = false;
countryClicked = false;

// set initial value for number of participants section of right column

d3.select("#num_participants").text(csv.length);

// create Age bar chart, giving id's to each bar
// add mouseover, mouseout, and click functionality to chart
// mouseover changes bar color and filters data for center chart
// mouseout resets color and resets values for center chart based on any other applied filters
// click changes bar color and filters data for all charts
    ...
    .on("mouseover", function() {
        if(!ageClicked) {
        var split_val = d3.select(this).attr("id");
        d3.select(this).attr("fill", "rgba(0,0,0,1)");
        if(genderClicked && countryClicked) {
        var new_data = gc_data.filter(function(d) {
            return d.Age_Bracket == split_val;
        })
    }
        else if (genderClicked) {
        var new_data = gender_data.filter(function(d) {
            return d.Age_Bracket == split_val;
        })
    }
        else if (countryClicked) {
        var new_data = country_data.filter(function(d) {
            return d.Age_Bracket == split_val;
        })
        }
        else {
...
// create gender bar chart adding mouseover, mouseout, and click functionality
// functionality is same as for age chart
    ...
    .on("mouseover", function() {
        ...
        var split_val = d3.select(this).attr("id");
        if (ageClicked && countryClicked) {
        var new_data = ac_data.filter(function(d) {
            return d.Gender == split_val;
        })
    }
        else if (ageClicked) {
        var new_data = age_data.filter(function(d) {
            return d.Gender == split_val;
        })
        }
        else if (countryClicked) {
...
// create country chart with mouseover, mouseout, and click functionality similar to that of other charts in left column
    ...
    .on("mouseover", function() {
        ...
        var split_val = d3.select(this).attr("id");
        if (ageClicked && genderClicked) {
        var new_data = ag_data.filter(function(d) {
...
// set initial value for candy click flag

candyClicked = false;

// create initial bar chart for candy based on all data
// colors are set so that a sentiment value over 0 has a green bar; a sentiment value under 0 has a red bar
// mouseover fills in right-column data and makes text black
// mouseout removes values from right column and grays out remaining text
// click preserves the selected candy to allow for enhanced filtering capabilities based on demographics
    ...
    .on("mouseover", function(d) {
        ...
        var overall_like = csv.filter(function(d) {
            return d[chosen_candy_col] == 1;
        }).length / csv.length;

        var overall_dislike = csv.filter(function(d) {
            return d[chosen_candy_col] == -1;
        }).length / csv.length;

        var overall_indifferent = csv.filter(function(d) {
            return d[chosen_candy_col] == 0;
        }).length / csv.length;

        var overall_like_rounded = d3.format(".2f")(overall_like * 100);
        var overall_dislike_rounded = d3.format(".2f")(overall_dislike * 100);
        var overall_indifferent_rounded = d3.format(".2f")(overall_indifferent * 100);
        ...
        if (csv.filter(function(d) {
           ...
           var young_like = csv.filter(function(d) {
                return d.Age_Bracket == "0-17" && d[candy_codes[i]] == 1;
            }).length / csv.filter(function(d) {
                return d.Age_Bracket == "0-17"}).length;

           var young_dislike = csv.filter(function(d) {
                return d.Age_Bracket == "0-17" && d[candy_codes[i]] == -1;
            }).length / csv.filter(function(d) {
                return d.Age_Bracket == "0-17"}).length;

            var young_like_rounded = d3.format(".2f")(young_like * 100);
            var young_dislike_rounded = d3.format(".2f")(young_dislike * 100);
            var young_text = d3.select("#young-like").text(young_like_rounded + "%");
           ...
           var twenties_like = csv.filter(function(d) {
                return d.Age_Bracket == "18-29" && d[candy_codes[i]] == 1;
            }).length / csv.filter(function(d) {
                return d.Age_Bracket == "18-29"}).length;

           var twenties_dislike = csv.filter(function(d) {
                return d.Age_Bracket == "18-29" && d[candy_codes[i]] == -1;
            }).length / csv.filter(function(d) {
                return d.Age_Bracket == "18-29"}).length;

            var twenties_like_rounded = d3.format(".2f")(twenties_like * 100);
            var twenties_dislike_rounded = d3.format(".2f")(twenties_dislike * 100);
           ...
           var thirties_like = csv.filter(function(d) {
                return d.Age_Bracket == "30-39" && d[candy_codes[i]] == 1;
            }).length / csv.filter(function(d) {
                return d.Age_Bracket == "30-39"}).length;

           var thirties_dislike = csv.filter(function(d) {
                return d.Age_Bracket == "30-39" && d[candy_codes[i]] == -1;
            }).length / csv.filter(function(d) {
                return d.Age_Bracket == "30-39"}).length;

            var thirties_like_rounded = d3.format(".2f")(thirties_like * 100);
            var thirties_dislike_rounded = d3.format(".2f")(thirties_dislike * 100);
           ...
           var forties_like = csv.filter(function(d) {
                return d.Age_Bracket == "40-49" && d[candy_codes[i]] == 1;
            }).length / csv.filter(function(d) {
                return d.Age_Bracket == "40-49"}).length;

           var forties_dislike = csv.filter(function(d) {
                return d.Age_Bracket == "40-49" && d[candy_codes[i]] == -1;
            }).length / csv.filter(function(d) {
                return d.Age_Bracket == "40-49"}).length;

            var forties_like_rounded = d3.format(".2f")(forties_like * 100);
            var forties_dislike_rounded = d3.format(".2f")(forties_dislike * 100);
           ...
           var fifties_like = csv.filter(function(d) {
                return d.Age_Bracket == "50-64" && d[candy_codes[i]] == 1;
            }).length / csv.filter(function(d) {
                return d.Age_Bracket == "50-64"}).length;

           var fifties_dislike = csv.filter(function(d) {
                return d.Age_Bracket == "50-64" && d[candy_codes[i]] == -1;
            }).length / csv.filter(function(d) {
                return d.Age_Bracket == "50-64"}).length;

            var fifties_like_rounded = d3.format(".2f")(fifties_like * 100);
            var fifties_dislike_rounded = d3.format(".2f")(fifties_dislike * 100);
           ...
           var seniors_like = csv.filter(function(d) {
                return d.Age_Bracket == "65+" && d[candy_codes[i]] == 1;
            }).length / csv.filter(function(d) {
                return d.Age_Bracket == "65+"}).length;

           var seniors_dislike = csv.filter(function(d) {
                return d.Age_Bracket == "65+" && d[candy_codes[i]] == -1;
            }).length / csv.filter(function(d) {
                return d.Age_Bracket == "65+"}).length;

            var seniors_like_rounded = d3.format(".2f")(seniors_like * 100);
            var seniors_dislike_rounded = d3.format(".2f")(seniors_dislike * 100);
           ...
           var men_like = csv.filter(function(d) {
                return d.Gender == "Male" && d[candy_codes[i]] == 1;
            }).length / csv.filter(function(d) {
                return d.Gender == "Male"}).length;

           var men_dislike = csv.filter(function(d) {
                return d.Gender == "Male" && d[candy_codes[i]] == -1;
            }).length / csv.filter(function(d) {
                return d.Gender == "Male"}).length;

            var men_like_rounded = d3.format(".2f")(men_like * 100);
            var men_dislike_rounded = d3.format(".2f")(men_dislike * 100);
           ...
           var women_like = csv.filter(function(d) {
                return d.Gender == "Female" && d[candy_codes[i]] == 1;
            }).length / csv.filter(function(d) {
                return d.Gender == "Female"}).length;

           var women_dislike = csv.filter(function(d) {
                return d.Gender == "Female" && d[candy_codes[i]] == -1;
            }).length / csv.filter(function(d) {
                return d.Gender == "Female"}).length;

            var women_like_rounded = d3.format(".2f")(women_like * 100);
            var women_dislike_rounded = d3.format(".2f")(women_dislike * 100);
           ...
           var gender_unknown_like = csv.filter(function(d) {
                return d.Gender == "Unknown" && d[candy_codes[i]] == 1;
            }).length / csv.filter(function(d) {
                return d.Gender == "Unknown"}).length;

           var gender_unknown_dislike = csv.filter(function(d) {
                return d.Gender == "Unknown" && d[candy_codes[i]] == -1;
            }).length / csv.filter(function(d) {
                return d.Gender == "Unknown"}).length;

            var gender_unknown_like_rounded = d3.format(".2f")(gender_unknown_like * 100);
            var gender_unknown_dislike_rounded = d3.format(".2f")(gender_unknown_dislike * 100);
           ...
           var americans_like = csv.filter(function(d) {
                return d.Country == "United States" && d[candy_codes[i]] == 1;
            }).length / csv.filter(function(d) {
                return d.Country == "United States"}).length;

           var americans_dislike = csv.filter(function(d) {
                return d.Country == "United States" && d[candy_codes[i]] == -1;
            }).length / csv.filter(function(d) {
                return d.Country == "United States"}).length;

            var americans_like_rounded = d3.format(".2f")(americans_like * 100);
            var americans_dislike_rounded = d3.format(".2f")(americans_dislike * 100);
           ...
           var canadians_like = csv.filter(function(d) {
                return d.Country == "Canada" && d[candy_codes[i]] == 1;
            }).length / csv.filter(function(d) {
                return d.Country == "Canada"}).length;

           var canadians_dislike = csv.filter(function(d) {
                return d.Country == "Canada" && d[candy_codes[i]] == -1;
            }).length / csv.filter(function(d) {
                return d.Country == "Canada"}).length;

            var canadians_like_rounded = d3.format(".2f")(canadians_like * 100);
            var canadians_dislike_rounded = d3.format(".2f")(canadians_dislike * 100);
           ...
           var other_countries_like = csv.filter(function(d) {
                return d.Country == "Other" && d[candy_codes[i]] == 1;
            }).length / csv.filter(function(d) {
                return d.Country == "Other"}).length;

           var other_countries_dislike = csv.filter(function(d) {
                return d.Country == "Other" && d[candy_codes[i]] == -1;
            }).length / csv.filter(function(d) {
                return d.Country == "Other"}).length;

            var other_countries_like_rounded = d3.format(".2f")(other_countries_like * 100);
            var other_countries_dislike_rounded = d3.format(".2f")(other_countries_dislike * 100);
            ...
            var sentiment = d.value;
            var rounded_sentiment = d3.format(".2f")(sentiment);
...
.on("click", function() {
    if (!candyClicked) {
        ...
        var clicked_val = d3.select(this).attr("id");
        if (clicked_val == chosen_candy) {
        candyClicked = false;
        chosen_candy = undefined;
    }
    }
...
// if a candy has been selected via clicking, adjust values in right column based on filtered demographic data
// set right-column value to "N/A" and gray it out if demographic does not exist in filtered data

if (candyClicked && typeof chosen_candy !== 'undefined') {
    var sentiment_val = candy_nest[chosen_candy];
    var sentiment_val_rounded = d3.format(".2f")(sentiment_val);
...
// create x- and y-scales for candy chart
...
// create candy chart with mouseover, mouseout, and click functionality
